...
app.innerHTML = `
  ...
  <main>
    <div class="config-banner" id="configBanner">
      ⚠ Kein API-Key konfiguriert. Füge <code>VITE_ETHERSCAN_API_KEY</code> in deiner <code>.env</code> Datei hinzu.
    </div>
    ...
    <div class="two-col">
      <div>
        ...
        <div class="wallets-grid" id="walletList">
      ...
      <div>
        ...
        <div class="feed-wrapper" id="liveFeed">
          <div class="placeholder">Warte auf Transaktionen…</div>
        </div>
      </div>
    </div>
  ...
  for (const w of TRACKED_WALLETS) {
    ...
    card.innerHTML = `
      <div class="wallet-top">
        <div>
          <div style="margin-bottom:4px"><span class="wallet-id">${shortAddr(w.address)}</span></div>
          <div class="wallet-label">${w.label}</div>
        </div>
        <div>
          <div class="wallet-bal" id="bal-${w.address}">…</div>
          <div class="wallet-bal-sub">ETH Balance</div>
        </div>
      </div>`
    list.appendChild(card)

    fetchBalance(w.address, API_KEY).then(bal => {
...
function addFeedRow(tx, walletLabel) {
  ...
  const isIn = tx.to &&
    TRACKED_WALLETS.some(w => w.address.toLowerCase() === tx.to.toLowerCase())

  const row = document.createElement('div')
  row.className = 'feed-row'
  row.innerHTML = `
    <span class="feed-time">${nowTime()}</span>
    <span class="feed-wallet">${walletLabel}</span>
    <span class="${isIn ? 'buy' : 'sell'}">${isIn ? '▲ IN' : '▼ OUT'}</span>
    <span class="feed-token">ETH</span>
    <span class="feed-value">${formatETH(tx.value)}</span>
    <span class="feed-age">${timeAgo(tx.timeStamp)} ago</span>`
...
async function pollTransactions() {
  for (const wallet of TRACKED_WALLETS) {
    const txs = await fetchWalletTxs(wallet.address, API_KEY)
    if (!txs) continue
...
async function updateBlock() {
  const block = await fetchLatestBlock(API_KEY)
  if (block) {
